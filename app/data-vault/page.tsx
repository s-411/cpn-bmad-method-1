      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="animate-fade-in space-y-8">
          ...
          <div className="card-cpn bg-gradient-to-br from-cpn-dark2 to-cpn-dark">
            ...
            {(filters.ethnicity !== 'All' || filters.hairColor !== 'All' || filters.ratingRange !== 'All' || filters.location !== 'All') && (
              <div className="mt-4 p-4 bg-cpn-yellow/10 border border-cpn-yellow/20 rounded-lg">
                <p className="text-sm text-cpn-white mb-2">
                  <strong>Active Filters:</strong>
                </p>
                <div className="flex flex-wrap gap-2">
                  {filters.ethnicity !== 'All' && (
                    <span className="px-2 py-1 bg-cpn-yellow/20 text-cpn-yellow rounded text-xs">
                      {filters.ethnicity}
                    </span>
                  )}
                  {filters.hairColor !== 'All' && (
                    <span className="px-2 py-1 bg-cpn-yellow/20 text-cpn-yellow rounded text-xs">
                      {filters.hairColor} hair
                    </span>
                  )}
                  {filters.ratingRange !== 'All' && (
                    <span className="px-2 py-1 bg-cpn-yellow/20 text-cpn-yellow rounded text-xs">
                      {filters.ratingRange} rating
                    </span>
                  )}
